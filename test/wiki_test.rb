        ...
        if File.exist?(wikifile) && w_size != 0
		 ...
		 if tmp_t[0] == "none"
			t = tmp_t[0]
		 	...
		 	#t = "https://twitter.com/" + tmp_t[0].gsub(/\*\s*\{\{\Twitter\|/,"").gsub(/\|(.+)/,"").gsub(/\}\}/,"").gsub(/\s*(.+)/,"").chomp
...
#                 client = Mysql2::Client.new(:host => 'localhost', :user => 'root', :password => 'rootpass')
		 puts "INSERT INTO girls.profile VALUES ('#{n}', '#{ymd}', '#{s}', '#{b}', '#{w}', '#{h}', '#{c}','#{t}');"
                 #client.query("INSERT IGNORE INTO idol.profile VALUES ('#{n}', '#{ymd}', '#{s}', '#{b}', '#{w}', '#{h}', '#{c}','#{t}');")
        else
